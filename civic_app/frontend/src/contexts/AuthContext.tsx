...
type AuthContextType = {
  ...
  loading: boolean;
  logout: () => Promise<void>;
  setAuthFromServer?: (jwt: string, user: AppUser | null) => void;
};
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const [token, setToken] = useState<string | null>(() => safeReadToken());
  const [loading, setLoading] = useState<boolean>(true);
  ...
  const setAuthFromServer = (jwt: string, userObj: AppUser | null) => {
    // only store valid non-empty strings
  ...
  const logout = useMemo(
    () => async (): Promise<void> => {
      try {
        ...
        setToken(null);
        setUser(null);
        // try to sign out Civic session if available
        try {
          if (civic && typeof (civic as any).signOut === "function") {
            await (civic as any).signOut();
          } else if (civic && typeof (civic as any).logout === "function") {
