...
const Header: React.FC<HeaderProps> = ({
  ...
  const handleCivicSignIn = () => {
    // First, try to trigger Civic authentication
    const civic = (window as any).civic;
    if (civic && typeof civic.triggerAuthFlow === "function") {
      civic.triggerAuthFlow();
    }

    // Then redirect to our handler
    setTimeout(() => {
      window.location.href = "/auth/civic/redirect";
    }, 1000);
  };
  return (
    <header
